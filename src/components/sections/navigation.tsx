...
export function Navigation() {
  ...
  return (
    <header
      className={`dark:bg-[#08080B] flex h-[56px] justify-between items-center fixed top-0 w-full z-50 bg-opacity-10 backdrop-blur-[10px] ${
        scrolled
          ? "border-b-[0.2px] border-gray-100 border-opacity-[10%]"
          : "border-b-[0.2px] border-gray-100 border-opacity-0"
      }`}
    >
      <Link href={"/"}>
        <div className={`ml-4 flex items-center desktop:ml-[200px]`}>
          <img src="/icon.png" className="w-12 h-12 select-none fade-in" alt="Logo" />
          <h1 className=" text-lg ml-2 select-none">danilppzz</h1>
        </div>
      </Link>
      <nav className={`flex items-center mr-4 space-x-4 desktop:mr-[200px] md:justify-end`}>
        ...
        <Link className="cursor-pointer hover:underline" href={"/docs"}>
          docs
        </Link>
